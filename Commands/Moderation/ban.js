  ...
  async execute(client, message, args) {
    ...
    try {
      ...
      const embed = new MessageEmbed()
        .setTitle('<:banhammer:865852725773991956> Successfully Banned User')
        .setColor('GREEN')
        .setThumbnail(member.user.displayAvatarURL({ format: "png", dynamic: true, size: 1024 }))
        .setDescription(`I have successfully banned ${member}\`(${member.id})\`\n\n**Reason:** ${reason}\n**Moderator:** ${message.author}`)

      const userEmbed = new MessageEmbed()
        .setTitle('<:banhammer:865852725773991956> You have been banned from ' + message.guild.name)
        .setColor('RED')
        .setThumbnail(member.user.displayAvatarURL({ format: "png", dynamic: true, size: 1024 }))
        .setDescription(`**Reason:** ${reason}\n**Moderator:** ${message.author}`)

      message.channel.send({ embeds: [embed] });
      member.send({ embeds: [userEmbed] }).catch(() => embed.setDescription(`I have successfully banned ${member}\`(${member.id})\`\n\n**Reason:** ${reason}\n**Moderator:** ${message.author}\n\n> I was unable to send messages to this user.`));
    } catch (error) {
